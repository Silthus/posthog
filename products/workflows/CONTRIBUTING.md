...
- A queue-parameter type that is not a plain fetch needs four edits, and skipping any one of them fails at run time rather than at build time. `llmGenerate` (the "Generate text" step) is the worked example:
  ...
  4. **Rescheduling.** A call that is still pending comes back as a reschedule, never as a wait: `createInvocationResult` clears `queueParameters`, `queueMetadata`, `queuePriority`, and `queueScheduledAt`, so re-assign the ones you need and set `result.finished = false`. `llmGenerate` restores its queue parameters, carries its state (generation id, wake token, execution count) on `queueMetadata`, and parks the job at backup marks 1, 3 and 5 minutes after acceptance (`BACKUP_POLL_MARKS_SECONDS`) for as long as the endpoint answers 202. On the terminal answer it pushes the result envelope onto `result.invocation.state.vmState.stack` itself.
  5. **Waking (optional).** A parked job does not have to wait out its schedule. The Django side of `llmGenerate` produces a `$workflows_llm_generation_finished` internal event when the generation reaches a terminal state, and `CdpHogflowSubscriptionMatcherConsumer` (already consuming `cdp_internal_events`) looks the parked job up by id, verifies the one-time `wakeToken` in the job's own `queueMetadata` against the event, and pulls it to `scheduled = NOW()`. The event is a wake-only signal - it carries no payload, the woken job re-enters the service and retrieves as usual - so a lost wake is never fatal: the backup marks above are the safety net, and the last one fails the step as `deadline_exceeded`.
