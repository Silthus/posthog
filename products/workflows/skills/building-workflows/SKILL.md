...
description: 'Build, edit, test, enable, and monitor PostHog workflows over MCP. Author the action/edge graph so it runs and opens cleanly in the visual editor, then change drafts surgically with patch operations. Use when asked to build, set up, automate, change, fix, or debug a workflow, campaign, broadcast, drip sequence, or event-triggered automation in the workflows product, including a step that uses AI or an LLM to generate text, write a subject line, or personalize a message.'
---
...
1. **Compose the graph.** Build `actions` + `edges` per [references/graph-schema.md](references/graph-schema.md). For any `function` node, don't guess the template: list the live catalog with `cdp-function-templates-list` and read its required inputs with `cdp-function-templates-retrieve`. The exceptions are the fixed literal ids, which you write from memory: `template-email` / `template-twilio` / `template-native-push` for the `function_email` / `function_sms` / `function_push` step types, and `template-workflow-llm` for the "Generate text" step.
2. **Create as a draft.** `workflows-create`. Every workflow is created `draft`; it does not execute yet.
3. **Test-run it.** `workflows-test-run` runs **one step at a time**. Start at the first step (omit `current_action_id`, or point it at the trigger) with sample `globals` (`{event, person, groups}`); the result includes the next step's id (`nextActionId`). Feed that back as `current_action_id` and run again, walking step by step to the end. Skip `delay` nodes by jumping to the action after them (delays aren't simulated). Async side effects (HTTP/email/SMS/push/text generation) are mocked unless you set `mock_async_functions=false`, and with mocking off the "Generate text" step runs a real generation that spends the project's PostHog AI credits. Read each step's trace to confirm the path taken.
