...
//
// Nothing here executes a workflow or talks to PostHog. A workflow file exports values
// built by this module and stops; the CLI loads the file and interprets what it finds.
// The two brands below are how the CLI tells a finished workflow from a chain the author
// forgot to close with .exit(), which is the difference between a file with one workflow
// in it and a file the CLI would otherwise report as empty.
...
export interface WebhookOptions {
    ...
    /** The HMAC key, named here and resolved from the environment at push. */
    readonly signingSecret?: SecretRef
}

export interface Chain {
    /** Marks an unfinished chain, so the CLI can name the missing .exit() call. */
    readonly kind: 'workflow_draft'
    delay(duration: Duration, options?: { name?: string }): Chain
    webhook(options: WebhookOptions): Chain
    branch(options: { name: string; branches: readonly [BranchSpec, ...BranchSpec[]] }): Chain
}

export interface Workflow {
    /** What the CLI looks for on every export of a workflow file. */
...
type Step =
    ...
    | { kind: 'webhook'; name: string; url: string; body?: Record<string, unknown>; signingSecret?: SecretRef }
    | { kind: 'branch'; name: string; branches: readonly BranchSpec[] }

class ChainImpl implements Chain {
    readonly kind = 'workflow_draft' as const
    readonly steps: Step[] = []
...
function compile(steps: readonly Step[], continuation: string, ids: Ids, actions: Action[], edges: Edge[]): string {
    if (steps.length === 0) {
        return continuation
    }
    ...
    steps.forEach((step, position) => {
        ...
        if (step.kind === 'webhook') {
            const inputs: FunctionInputs = {
                ...
                ...(step.signingSecret ? { signing_secret: { value: step.signingSecret } } : {}),
            }
            actions.push({
                id,
                name: step.name,
...
export interface WorkflowChain extends Chain {
    ...
    webhook(options: WebhookOptions): WorkflowChain
