...
// /api/projects/{id}/hog_flows/ accepts. Hand-copied from
...
/** Trigger config, discriminated on `type`. The prototype covers `event` only. */
export type TriggerConfig = {
    readonly type: 'event'
    readonly filters: ActionFilters
}
...
/**
 * A secret named in the source and resolved from the deployer's environment at push.
 * It travels inside the emitted definition as a marker, so `check` can report which
 * inputs are secret without ever needing the value.
 */
...
/** Action config, discriminated on the owning action's `type`. */
export type ActionConfig =
    | TriggerConfig
    | { readonly delay_duration: Duration }
    | { readonly conditions: readonly BranchCondition[] }
    | { readonly template_id: string; readonly inputs: FunctionInputs }
    | { readonly reason?: string }

export type ActionType = 'trigger' | 'delay' | 'conditional_branch' | 'function' | 'exit'
